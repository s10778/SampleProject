...
class Logger {
    ...
    private function out($level, $msg) {
        if(Config::IS_LOGFILE) {
            ...
            $logFilePath = Env::LOGDIR_PATH . Config::LOGFILE_NAME . '.log';
            ...
            if(Config::LOGFILE_MAXSIZE < filesize($logFilePath)) {
                ...
                $oldPath = Env::LOGDIR_PATH . Config::LOGFILE_NAME . '_' . date('YmdHis');
                    ...
                    while (($fileName = readdir($dh)) !== false) {
                        ...
                        if($pm === 1) {
                            ...
                            if($logCreatedDate < $retentionDate) {
                                unlink(Env::LOGDIR_PATH . '/' . $fileName);
                            }
                        }
                    }
                    closedir($dh);
